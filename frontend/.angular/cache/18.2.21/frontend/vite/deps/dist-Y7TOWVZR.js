...
var argList = new ExternalTokenizer((input, stack) => {
  if (input.next == 40) {
    ...
    if (isAlpha(prev) || isDigit(prev) || prev == 95 || prev == 45) input.acceptToken(openArgList, 1);
  }
});
var descendant = new ExternalTokenizer((input) => {
  if (space.indexOf(input.peek(-1)) > -1) {
    ...
    if (isAlpha(next) || next == 95 || next == 35 || next == 46 || next == 91 || next == 58 || next == 45) input.acceptToken(descendantOp);
  }
});
...
var parser = LRParser.deserialize({
  ...
  maxTerm: 133,
  nodeProps: [["isolate", -3, 3, 4, 30, ""], ["openedBy", 17, "(", 59, "{"], ["closedBy", 26, ")", 60, "}"]],
...
function less() {
  return new LanguageSupport(lessLanguage, lessLanguage.data.of({
...
//# sourceMappingURL=dist-Y7TOWVZR.js.map
