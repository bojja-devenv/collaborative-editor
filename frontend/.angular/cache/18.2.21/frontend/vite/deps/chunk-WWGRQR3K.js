...
var Parse = class {
  ...
  advanceStack(stack, stacks, split) {
    ...
    if (defaultReduce > 0) {
      ...
      if (verbose) console.log(base + this.stackID(stack) + ` (via always-reduce ${parser.getName(
        defaultReduce & 65535
        /* Action.ValueMask */
      )})`);
      return true;
    ...
    for (let i = 0; i < actions.length; ) {
      ...
      if (verbose) console.log(base + this.stackID(localStack) + ` (via ${(action & 65536) == 0 ? "shift" : `reduce of ${parser.getName(
        action & 65535
        /* Action.ValueMask */
  ...
  */
  parseDialect(dialect) {
    ...
    for (let i = 0; i < values.length; i++) if (!flags[i]) {
      for (let j = this.dialects[values[i]], id2; (id2 = this.data[j++]) != 65535; ) (disabled || (disabled = new Uint8Array(this.maxTerm + 1)))[id2] = 1;
    }
...
//# sourceMappingURL=chunk-WWGRQR3K.js.map
