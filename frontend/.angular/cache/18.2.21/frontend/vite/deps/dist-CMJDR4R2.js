...
} from "./chunk-4SOXAAPX.js";
import "./chunk-BZMFXI5N.js";
import "./chunk-B2WYVAOE.js";
import "./chunk-WWGRQR3K.js";
import {
  CompletionContext
...
var DefaultEndLeaf = [(_, line) => isAtxHeading(line) >= 0, (_, line) => isFencedCode(line) >= 0, (_, line) => isBlockquote(line) >= 0, (p, line) => isBulletList(line, p, true) >= 0, (p, line) => isOrderedList(line, p, true) >= 0, (p, line) => isHorizontalRule(line, p, true) >= 0, (p, line) => isHTMLBlock(line, p, true) >= 0];
var scanLineResult = {
  text: "",
  end: 0
};
...
function finishLink(cx, content, type, start, startPos) {
  let {
    text
  } = cx, next = cx.char(startPos), endPos = startPos;
...
function parseCode(config) {
  ...
  let wrap = parseMixed((node, input) => {
    ...
    if (codeParser && (id == Type.CodeBlock || id == Type.FencedCode)) {
      ...
      if (parser2) return {
        parser: parser2,
        overlay: (node2) => node2.type.id == Type.CodeText
      };
...
function autolinkURLEnd(text, from) {
  ...
  for (; ; ) {
    ...
    else if (last == ";" && (m2 = /&(?:#\d+|#x[a-f\d]+|\w+);$/.exec(text.slice(from, end)))) end = from + m2.index;
    else break;
  }
  return end;
}
...
var commonmark = parser.configure({
  ...
  }), headingProp.add(isHeading), indentNodeProp.add({
    Document: () => null
...
var insertNewlineContinueMarkupCommand = (config = {}) => ({
  state,
  dispatch
}) => {
  ...
  let dont = null, changes = state.changeByRange((range) => {
    if (!range.empty || !markdownLanguage.isActiveAt(state, range.from, -1) && !markdownLanguage.isActiveAt(state, range.from, 1)) return dont = {
      range
    };
    ...
    if (inner.item && emptyLine) {
      ...
      if (first.to >= pos || second && second.to < pos || line.from > 0 && !/[^\s>]/.test(doc.lineAt(line.from - 1).text) || config.nonTightLists === false) {
        ...
        return {
          ...
          changes: {
            from: line.from,
            insert: insert2 + state.lineBreak
          }
        };
      }
    }
...
function htmlTagCompletion(context) {
  let {
    state,
    pos
  } = context, m = /<[:\-\.\w\u00b7-\uffff]*$/.exec(state.sliceDoc(pos - 25, pos));
  if (!m) return null;
...
//# sourceMappingURL=dist-CMJDR4R2.js.map
