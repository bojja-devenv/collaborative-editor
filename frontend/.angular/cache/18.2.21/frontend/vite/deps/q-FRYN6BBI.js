...
function tokenBase(stream, state) {
  ...
  if (sol) {
    if (c == "/") return (state.tokenize = tokenLineComment)(stream, state);
      ...
      if (stream.eol() || /\s/.test(stream.peek())) return stream.skipToEnd(), /^\\\s*$/.test(stream.current()) ? (state.tokenize = tokenCommentToEOF)(stream) : state.tokenize = tokenBase, "comment";
...
//# sourceMappingURL=q-FRYN6BBI.js.map
