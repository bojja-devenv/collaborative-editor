    ...
    for (let parent = this; ; ) {
      for (let {
        ...
        if (next instanceof TreeBuffer) {
          ...
          let mounted;
          if (!(mode & IterMode.IgnoreMounts) && (mounted = MountedTree.get(next)) && !mounted.overlay) return new _TreeNode(mounted.tree, start, i, parent);
  ...
  enter(pos, side, mode = 0) {
    ...
    if (!(mode & IterMode.IgnoreOverlays) && (mounted = MountedTree.get(this._tree)) && mounted.overlay) {
      ...
      for (let {
        ...
        if ((side > 0 ? from <= rPos : from < rPos) && (side < 0 ? to >= rPos : to > rPos)) return new _TreeNode(mounted.tree, mounted.overlay[0].from + this.from, -1, this);
      }
    }
      ...
      if (this.stoppedAt != null && cursor.from >= this.stoppedAt) {
        ...
        if (overlay) {
          ...
          if (match) for (let r of match.mount.overlay) {
            ...
            if (from >= cursor.from && to <= cursor.to && !overlay.ranges.some((r2) => r2.from < to && r2.to > from)) overlay.ranges.push({
              from,
              to
            });
          }
...
var Tag = class _Tag {
  ...
  */
  ...
  static define(nameOrParent, parent) {
    ...
    if (nameOrParent instanceof _Tag) parent = nameOrParent;
...
var Modifier = class _Modifier {
  ...
  static get(base, mods) {
    ...
    for (let parent of base.set) if (!parent.modified.length) for (let config of configs) set.push(_Modifier.get(parent, config));
    return tag;
  }
};
...
function indentOnInput() {
  return EditorState.transactionFilter.of((tr) => {
    ...
    let doc = tr.newDoc, {
      head
    } = tr.newSelection.main, line = doc.lineAt(head);
...
function foldGutter(config = {}) {
  let fullConfig = __spreadValues(__spreadValues({}, foldGutterDefaults), config);
...
var TreeHighlighter = class {
  ...
  update(update) {
    ...
    let {
      viewport
    } = update.view, decoratedToMapped = update.changes.mapPos(this.decoratedTo, 1);
...
function countCol(string2, end, tabSize, startIndex = 0, startValue = 0) {
  ...
  for (let i = startIndex; i < end; i++) {
    if (string2.charCodeAt(i) == 9) n += tabSize - n % tabSize;
    else n++;
  }
  return n;
}
...
//# sourceMappingURL=chunk-WW2G6J3Q.js.map
