...
function rawTokenizer(endTag, text2, tagStart2) {
  return new ExternalTokenizer((input) => {
    ...
    if (input.pos > start) input.acceptToken(text2);
  });
}
...
var parser = LRParser.deserialize({
  ...
  maxTerm: 173,
  nodeProps: [["closedBy", 1, "}}", 2, "#}", -2, 3, 4, "%}", 32, ")"], ["openedBy", 7, "{{", 31, "(", 57, "{%", 140, "{#"], ["group", -18, 9, 10, 13, 14, 21, 22, 25, 27, 38, 41, 42, 43, 44, 45, 49, 50, 51, 52, "Expression", -11, 53, 64, 71, 77, 84, 92, 97, 102, 107, 114, 119, "Statement"]],
  skippedNodes: [0],
  repeatNodeCount: 13,
...
function findContext(context) {
  ...
  if (context.explicit && (before == "FilterOp" || before == "filter")) return {
    type: "filter"
  };
...
function jinjaCompletionSource(config = {}) {
  ...
  return (context) => {
    ...
    else if (cx.type == "tag") options = tags2;
...
var closePercentBrace = EditorView.inputHandler.of((view, from, to, text2) => {
  if (text2 != "%" || from != to || view.state.doc.sliceString(from - 1, to + 1) != "{}") return false;
  ...
  parser: parser.configure({
    props: [styleTags({
      ...
      "Statement Comment"(tree) {
        ...
        return {
          from: first.to,
          to: last.name == "EndTag" || last.name == "#}" ? last.from : tree.to
        };
      }
    })]
  }),
...
function jinja(config = {}) {
  ...
  return new LanguageSupport(lang, [base2.support, lang.data.of({
    autocomplete: jinjaCompletionSource(config)
  }), base2.language.data.of({
    closeBrackets: {
      brackets: ["{"]
    }
  }), closePercentBrace]);
...
//# sourceMappingURL=dist-M7G3CFPW.js.map
