...
function mkJavaScript(parserConfig) {
  ...
  return {
    ...
    indent: function(state, textAfter, cx2) {
      ...
      else if (type2 == "stat") return lexical.indented + (isContinuedStatement(state, textAfter) ? statementIndent || cx2.unit : 0);
      else if (lexical.info == "switch" && !closing && parserConfig.doubleIndentSwitch != false) return lexical.indented + (/^(?:case|default)\b/.test(textAfter) ? cx2.unit : 2 * cx2.unit);
...
var javascript = mkJavaScript({
  name: "javascript"
});
var json = mkJavaScript({
  name: "json",
  json: true
});
...
//# sourceMappingURL=chunk-HJGSCAUL.js.map
