...
var typescriptSnippets = snippets.concat([snippetCompletion("interface ${name} {\n	${}\n}", {
  label: "interface",
  detail: "definition",
  type: "keyword"
...
var autoCloseTags = EditorView.inputHandler.of((view, from, to, text, defaultInsert) => {
  ...
  let closeTags = state.changeByRange((range) => {
    ...
    let {
      head
    } = range, around = syntaxTree(state).resolveInner(head - 1, -1), name;
...
function translateDiagnostic(input, doc, offset) {
  ...
  if (input.fix) {
    let {
      range,
      text
    } = input.fix, from = range[0] + offset.pos - start, to = range[1] + offset.pos - start;
    result.actions = [{
      name: "fix",
      apply(view, start2) {
        view.dispatch({
...
//# sourceMappingURL=chunk-B2WYVAOE.js.map
