...
function insertCompletionText(state, text, from, to) {
  let {
    main
  } = state.selection, fromOff = from - main.from, toOff = to - main.from;
    ...
    if (a.result.filter === false) {
      ...
      for (let option of a.result.options) if (match = matcher.match(option.label)) {
        ...
        if (typeof option.section == "object" && option.section.rank === "dynamic") {
          ...
          if (!dynamicSectionScore) dynamicSectionScore = /* @__PURE__ */ Object.create(null);
...
var CompletionDialog = class _CompletionDialog {
  ...
  map(changes) {
    return new _CompletionDialog(this.options, this.attrs, __spreadProps(__spreadValues({}, this.tooltip), {
...
var CompletionState = class _CompletionState {
  ...
  update(tr) {
    let {
      state
    } = tr, conf = state.facet(completionConfig);
...
var ActiveSource = class _ActiveSource {
  ...
  update(tr, conf) {
    ...
    if (type & 8 || type & 16 && this.touches(tr)) value = new _ActiveSource(
      value.source,
      0
      /* State.Inactive */
    );
...
var ActiveResult = class _ActiveResult extends ActiveSource {
  ...
  updateFor(tr, type) {
    ...
    if (result.update && (result = result.update(result, from, to, new CompletionContext(tr.state, pos, false)))) return new _ActiveResult(this.source, this.explicit, limit, result, result.from, (_a = result.to) !== null && _a !== void 0 ? _a : cur(tr.state));
...
function applyCompletion(view, option) {
  ...
  if (typeof apply == "string") view.dispatch(__spreadProps(__spreadValues({}, insertCompletionText(view.state, apply, result.from, result.to)), {
...
function autocompletion(config2 = {}) {
  return [commitCharacters, completionState, completionConfig.of(config2), completionPlugin, completionKeymapExt, baseTheme];
}
...
//# sourceMappingURL=chunk-XM4KIF7E.js.map
