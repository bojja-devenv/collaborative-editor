...
var TextView = class _TextView extends ContentView {
  ...
  merge(from, to, source) {
    if (this.flags & 8 || source && (!(source instanceof _TextView) || this.length - (to - from) + source.length > MaxJoinLen || source.flags & 8)) return false;
...
var MarkView = class _MarkView extends ContentView {
  ...
  merge(from, to, source, _hasStart, openStart, openEnd) {
    if (source && (!(source instanceof _MarkView && source.mark.eq(this.mark)) || from && openStart <= 0 || to < this.length && openEnd <= 0)) return false;
...
var WidgetView = class _WidgetView extends ContentView {
  ...
  merge(from, to, source, hasStart, openStart, openEnd) {
    if (source && (!(source instanceof _WidgetView) || !this.widget.compare(source.widget) || from > 0 && openStart <= 0 || to < this.length && openEnd <= 0)) return false;
...
var BlockWidgetView = class _BlockWidgetView extends ContentView {
  ...
  merge(from, to, source, _takeDeco, openStart, openEnd) {
    if (source && (!(source instanceof _BlockWidgetView) || !this.widget.compare(source.widget) || from > 0 && openStart <= 0 || to < this.length && openEnd <= 0)) return false;
    this.length = from + (source ? source.length : 0) + (this.length - to);
    return true;
  }
  domAtPos(pos) {
...
function posAtCoords(view, coords, precise, bias = -1) {
  ...
  if (element && ((_a = view.docView.nearest(element)) === null || _a === void 0 ? void 0 : _a.isEditable) != false) {
    ...
    if (node && (!view.contentDOM.contains(node) || browser.safari && isSuspiciousSafariCaretResult(node, offset, x) || browser.chrome && isSuspiciousChromeCaretResult(node, offset, x))) node = void 0;
...
function computeHandlers(plugins) {
  ...
  for (let plugin2 of plugins) {
    ...
    if (handlers2) for (let type in handlers2) {
      ...
      if (f) record(type).handlers.push(bindHandler(plugin2.value, f));
    }
    if (observers2) for (let type in observers2) {
      let f = observers2[type];
      if (f) record(type).observers.push(bindHandler(plugin2.value, f));
    }
  }
...
var MouseSelection = class {
  ...
  select(event) {
    let {
      view
    } = this, selection = skipAtomsForSelection(this.atoms, this.style.get(event, this.extend, this.multiple));
...
function doPaste(view, input) {
  ...
  let {
    state
  } = view, changes, i = 1, text = state.toText(input);
...
var EditorView = class _EditorView {
  ...
  measure(flush = true) {
    ...
    try {
      for (let i = 0; ; i++) {
        ...
        for (let i2 = 0; i2 < measuring.length; i2++) if (measured[i2] != BadMeasure) {
          try {
            let m = measuring[i2];
  ...
  */
  ...
  showAnnouncements(trs) {
    let first = true;
    for (let tr of trs) for (let effect of tr.effects) if (effect.is(_EditorView.announce)) {
  ...
  */
  plugin(plugin2) {
    ...
    if (known === void 0 || known && known.plugin != plugin2) this.pluginMap.set(plugin2, known = this.plugins.find((p) => p.plugin == plugin2) || null);
...
var CachedOrder = class _CachedOrder {
  ...
  static update(cache, changes) {
    ...
    for (let i = Math.max(0, cache.length - 10); i < cache.length; i++) {
      let entry = cache[i];
      if (entry.dir == lastDir && !changes.touchesRange(entry.from, entry.to)) result.push(new _CachedOrder(changes.mapPos(entry.from, 1), changes.mapPos(entry.to, -1), entry.dir, entry.isolates, false, entry.order));
    }
    return result;
  }
};
...
function modifiers(name, event, shift2) {
  ...
  if (shift2 !== false && event.shiftKey) name = "Shift-" + name;
  return name;
}
...
function buildKeymap(bindings, platform = currentPlatform) {
  ...
  for (let b of bindings) {
    ...
    for (let scope of scopes) {
      ...
      if (b.shift) add2(scope, "Shift-" + name, b.shift, b.preventDefault, b.stopPropagation);
    }
  }
  return bound;
}
...
function rectanglesForRange(view, className, range) {
  ...
  function pieces({
    ...
    let pieces2 = [];
    for (let i = 0; i < horizontal.length; i += 2) pieces2.push(piece(horizontal[i], top2, horizontal[i + 1], bottom));
    return pieces2;
  }
  ...
  */
  createDeco(view) {
    ...
    for (let {
      from,
      to
    } of matchRanges(view, this.maxLength)) iterMatches(view.state.doc, this.regexp, from, to, (from2, m) => this.addMatch(m, view, from2, add2));
    return build.finish();
  }
  /**
...
function placeholder(content) {
  ...
  return typeof content == "string" ? [plugin2, EditorView.contentAttributes.of({
...
function crosshairCursor(options = {}) {
  ...
  return [plugin2, EditorView.contentAttributes.of((view) => {
    var _a;
...
var tooltipPlugin = ViewPlugin.fromClass(class {
  ...
  writeMeasure(measured) {
    ...
    for (let i = 0; i < this.manager.tooltips.length; i++) {
      let tooltip = this.manager.tooltips[i], tView = this.manager.tooltipViews[i], {
        dom
      } = tView;
...
var HoverPlugin = class {
  ...
  mousemove(event) {
    ...
    if (active.length && tooltip && !isInTooltip(tooltip.dom, event) || this.pending) {
      let {
        pos
      } = active[0] || this.pending, end = (_b = (_a = active[0]) === null || _a === void 0 ? void 0 : _a.end) !== null && _b !== void 0 ? _b : pos;
...
function gutter(config) {
  return [gutters(), activeGutters.of(__spreadValues(__spreadValues({}, defaults), config))];
}
...
//# sourceMappingURL=chunk-W6G24EJV.js.map
